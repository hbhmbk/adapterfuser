...
class StableDiffusionXLAdapterPipeline(
    ...
    def __call__(
        ...
        b,c,h,w = list(image.values())[0].shape
        # height, width = self._default_height_width(height, width, image)
        height, width=h,w
        device = self._execution_device

        # if isinstance(self.adapters):
        #     img=_preprocess_adapter_image(image, height, width)
        adapter_feature=self.adapters.get_features(image)
        ...
        #     adapter_input = _preprocess_adapter_image(image, height, width)
        #     adapter_input = adapter_input.to(device=device, dtype=self.adapter.dtype)
        #     # print(adapter_input.mean())
        #     adapter_state = self.adapter(adapter_input)
        #     for k,v in enumerate(adapter_state):
        #         # adapter_state[k]=adapter_state[k].to(dtype=torch.bfloat16)
        #         adapter_state[k] = torch.cat([adapter_state[k]] * 2, dim=0)
        # else:adapter_state=None
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # adapter_state=self.adapter_fuser(self.scheduler.scale_model_input(latents, t),prompt_embeds,t.expand(1),additional_residuals=adapter_feature,keep_cond=keep_cond)
                # adapter_state=self.adapter_fuser(adapter_feature,t=t.expand(1),keep_cond=keep_cond)
                adapter_state=self.adapter_fuser(latent=self.scheduler.scale_model_input(latents, t).float(),context=adapter_feature,temb=t.expand(1),keep_cond=keep_cond)
                ...
                if i < int(num_inference_steps * adapter_conditioning_factor):
                    down_block_additional_residuals = [torch.cat([d]*2).to(dtype=self.unet.dtype) for d in adapter_state]
                    # down_block_additional_residuals = [torch.cat([d]*2).to(dtype=self.unet.dtype) for d in adapter_state]
                    # down_block_additional_residuals = [state.clone().to(dtype=self.unet.dtype) for state in adapter_state]
                else:
